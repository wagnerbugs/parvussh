...
**Agreed on 2026-08-06, to be studied before it is started.** Not a packaging
chore: the sandbox asks two real design questions, and they are the milestone.

**Why it is worth it.** The GTK 4.12 floor is what keeps Debian stable and Mint
users out, and no amount of code fixes that — their GTK is simply older. A
Flatpak carries its own runtime, so the host's version stops mattering. This
removes the project's largest practical limit, which is reach, not quality.

**What already exists**, done deliberately at M14 with this day in mind: the
two data filenames match `APP_ID` exactly, `metainfo.xml` validates apart from
the unreachable URLs, screenshots are generated by `make screenshots`, the
license is declared, and the string `OWNER` appears nowhere.

### The two questions the sandbox asks
...
2. **Running `ssh` and `ssh-keygen`.** There is no host openssh inside the
   ...
   - **Bundle `openssh-client` in the manifest.** Self-contained, but the
     ...
     each is granted. `validate()` (`ssh -G` on a temp file) is safe here —
     it reads nothing of the user's. The connection test and key generation
     are where the difference is felt.
   - **`flatpak-spawn --host`.** Runs the real one, with the user's agent and
     hosts file, at the cost of a permission that largely reopens the sandbox.
   Decide with the failure modes written down, and record it in
   `docs/DECISIONS.md` as D5 — this is exactly the kind of choice that file
   exists for.
